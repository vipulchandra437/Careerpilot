...
} from "@/lib/prompts/resumeParse";
import { parseRequestSchema, parsedResumeSchema } from "@/lib/validators/resume";
...
export async function POST(
  ...
  // 4. Guard: empty rawText means extraction failed at upload time
  if (!resume.rawText.trim()) {
    return NextResponse.json(
      {
        error:
          "This resume has no extractable text — try uploading a different file.",
        code: "EMPTY_SOURCE",
      },
      { status: 400 }
    );
  ...
  try {
    llmRaw = await askBrain(
      buildResumeParsePrompt(resume.rawText),
      RESUME_PARSE_SYSTEM_PROMPT,
      "resume-parse"
    );
  } catch {
  ...
  if (!jsonSubstring) {
    // WHY null not crash: a total parse failure is a known edge case. Save
    // parsedData = null so the UI shows "Failed" status and a retry button,
    // rather than a 500 error.
    await prisma.resume.update({
      where: { id: resumeId },
            data: { parsedData: { set: null } },
    });
    return NextResponse.json(
      {
        error: "Couldn't quite read that resume — try re-parsing.",
  ...
  // upload); we're only populating the parsedData column.
  await prisma.resume.update({
    where: { id: resumeId },
    data: { parsedData: parsedJson as Prisma.InputJsonValue },
  });

  return NextResponse.json({ ok: true, parsed: parsedJson }, { status: 200 });
}
