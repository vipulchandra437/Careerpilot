...
/**
 * True when `command` looks like a path (absolute or containing separators),
 * not a bare executable name to be resolved against `node_modules/.bin`.
 */
...
function main(argv) {
  ...
  let runArgs = args;
  if (isWin32 && !looksLikePath(command)) {
    // A wrapped `node` has no `.bin` stub to resolve — run THIS node.
