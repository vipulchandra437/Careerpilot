...
- [x] Free-tier limits enforced (1 gap analysis, 3 practice challenges, 1 mock interview, 1 roadmap before requiring credits) — **implemented + E2E-demonstrated in Task 2 (P6-7) and the Phase 6 sign-off walk (P6-15):** free uses consumed → blocked past limit → 402 style InsufficientCredits. Frontend user-facing "purchase credits to continue" messaging is a tracked launch-polish follow-up (see MEMORY.md).
...
- [x] **Phase 6 SIGNED OFF by user (P6-15).** User reviewed the real E2E payment-correctness walk: free-tier block, Starter $5/50 purchase 0→50, idempotent webhook retry (50, still 50), exact paid deduction (50→48→46), insufficient block with no negative/negative/free-ride, refund restore, and audit invariant balance==sum(ledger deltas). All items done.
**Exit criteria:** a new user can sign up, use the free tier, hit a limit, purchase credits, and have the correct balance deducted on next use — **verified end to end (P6-15).**
...
- [x] Full eval sets reviewed for all LLM-driven features (gap analysis, roadmap, challenges, interviews, feedback). **DONE — all 5 features have documented eval-set entries (RULES.md §4) + deterministic regression guards.** Pre-existing: feedback (verbatim-quote anchor), gap-SQL (reason stability), interview (concrete-detail enforcement). **Added this phase:** gap-beyond-SQL (multi-skill non-fabrication, `TestEvalSetBeyondSql`), roadmap (gap-linking/sequenced/actionable, `test_roadmap_eval.py`), coding challenges (stdlib/self-contained/first-test-matches-example, `TestEvalSetChallenge`). Full suite 160 → 177 pass.
...
- [x] Basic load test on the sandbox execution service (concurrent submissions). **DONE (P7-1):** real Judge0 executor path (`backend/sandbox/executor.py run_code`), 5-test multiplication program. 12 concurrent → 12/12 passed, **0 infra failures**, p50 5.7s / p95 6.2s. 30 concurrent → **30/30 passed, 0 infra failures**, p50 9.5s / p95 11.7s, throughput ~2.6 submissions/s (victim of public-CE rate limits, not our code — no SandboxRunError at any concurrency, and all results confirmed correct). No direct-execution fallback exists (RULES §2), so infra failure degrades to a clean 503 rather than unsandboxed execution.
- [x] Security review against RULES.md §2 as a checklist. **DONE (P7-2): all 4 §2 rules reviewed against the codebase.** (1) Sandbox-only execution — PASS: only `os`/`subprocess` usage in repo is the in-container `sandbox/runner.py`; every code path routes through Judge0 `executor.run_code` and fails closed to 503 on infra error, no exec/eval of student code in the API process. (2) Uploads private by default — PASS: storage returns keys not URLs, `file_storage.get_presigned_url` enforces `profiles/<user_id>/` ownership prefix, and no file-serving route exists that bypasses auth. (3) GitHub tokens encrypted at rest & never logged — **fixed:** was a homegrown XOR cipher (keystream-reuse/known-plaintext risk) → migrated to **Fernet** (authenticated AEAD) with a dedicated `GITHUB_TOKEN_ENCRYPTION_KEY` (`config.py` + `services/github.py`); logging-side redaction already solid (`main.py` RedactSensitiveQueryParams blanks `code`/`state`/`token` etc.). (4) Admin routes role-gated server-side — PASS: `dependencies.require_admin` on all 14 admin endpoints, enforced by tests.
